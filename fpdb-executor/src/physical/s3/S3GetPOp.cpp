...
std::shared_ptr<TupleSet>  S3GetPOp::s3GetFullRequest() {
  ...
  while (true) {
    ...
    auto duration = duration_cast<std::chrono::milliseconds>(chrono::steady_clock::now() - startTime).count();
    ////SPDLOG_CRITICAL("[{}]: Duration = {} sec", name(), duration*1e-3);
...
std::shared_ptr<TupleSet>  S3GetPOp::readTuples(){
  ...
  //SPDLOG_CRITICAL("Finished Reading for {}, num of rows {}, timestamp {}", name(), readTupleSet->size(), timestamp);
  return readTupleSet;
}
